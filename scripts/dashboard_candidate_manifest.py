...
def assemble(targets_dir, browsers_path, results_path, inventory_path, maintainer, approved_by):
    ...
    evidence.require(len(results) == len(evidence.REQUIRED_RELEASE_GATES) and {item.get("gate") for item in results} == evidence.REQUIRED_RELEASE_GATES, "results evidence must contain every required release gate")
