...
pub fn export(path: &Path, include_review_audit: bool) -> ExportResponse {
    ...
    let queries=[
      ...
      ExportQuery{name:"cdt_provider_enrichments",sql:"SELECT event_id,operation_revision_id,provider_evidence_json,reviewer_terms_json,created_at FROM cdt_provider_enrichments ORDER BY event_id"},
      ...
      ExportQuery{name:"investment_provenance",sql:"SELECT p.id,p.investment_document_event_id,p.investment_snapshot_id,p.cdt_operation_revision_id,p.source_document_id,p.import_batch_id,p.page_number,p.row_index,p.extractor_name,p.extractor_version,p.parser_id,p.parser_version,p.evidence_redaction,p.evidence_text_redacted,p.raw_storage_policy,p.confidence,p.created_at FROM provenance p LEFT JOIN investment_document_events e ON e.id=p.investment_document_event_id WHERE p.investment_snapshot_id IS NOT NULL OR p.cdt_operation_revision_id IS NOT NULL OR e.status='accepted' ORDER BY COALESCE(p.investment_document_event_id,p.investment_snapshot_id,p.cdt_operation_revision_id),p.id"},
