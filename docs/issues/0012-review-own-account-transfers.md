...
- [ ] Tracky can list likely own-account transfer/card-payment pairs as JSON using date, amount, owned account metadata, and semantic hints.
- [ ] A review command can accept a pair as a transfer/card payment.
- [ ] Accepted transfer pairs preserve provenance for both source candidates.
- [ ] Accepted transfer pairs do not inflate income or expense totals.
- [ ] The command refuses to pair candidates that are already accepted/rejected or belong to unresolved non-owned accounts.
- [ ] Tests cover a Nequi-to-RappiCard PSE payment pair and a non-match case.
